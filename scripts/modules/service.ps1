function Configure-WindowsService {
    ...
    Write-Info "Verificando privilegios de administrador para modo Servico..."
    if (-not (Test-IsAdmin)) {
        Write-Error "Modo Servico requer privilegios de administrador"
        Write-Info "Execute o script como Administrador ou escolha o modo Processo"
        Stop-Script 1
    }
    Write-Success "Privilegios de administrador confirmados"
    ...
    if ($useNssm) {
        try {
            ...
            & $nssmExe set SS54Backend AppParameters "-m uvicorn app.main:app --host 127.0.0.1 --port $port" 2>&1 | Out-Null
            ...
            Write-Info "Servico configurado para acessar apenas via localhost (127.0.0.1:$port)"
            Write-Info "Use Cloudflare Tunnel para acesso externo"
        } catch {
