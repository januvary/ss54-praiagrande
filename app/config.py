...
class Settings(BaseSettings):
    ...
    MAX_CONCURRENT_IMAGE_CONVERSIONS: int = (
        1  # Max simultaneous image to PDF conversions (serial processing for lower CPU)
    )
    ...
    BATCH_SEND_HOUR: int = 17
    BATCH_SEND_ENABLED: bool = False
    DRS_FOLLOWUP_HOUR: int = 14
    DRS_FOLLOWUP_ENABLED: bool = False
    AUTO_EXPIRE_HOUR: int = 10
    AUTO_EXPIRE_ENABLED: bool = False
    ...
    # Sync Configuration (Google Drive via Rclone)
    SYNC_RCLONE_REMOTE: str = "gdrive"  # Rclone remote name (configured interactively)
    SYNC_RCLONE_PATH: str = "ss54-archives"  # Folder name in Google Drive
    SYNC_RCLONE_CONFIG: str = (
        "/opt/ss54-backend/.rclone.conf"  # Rclone config file path
    )
    SYNC_RETRY_INTERVAL_MINUTES: int = 15
