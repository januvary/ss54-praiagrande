...
def update_process_status(
    ...
    if new_status in TERMINAL_STATUSES and old_status not in [
        s.value for s in TERMINAL_STATUSES
    ]:
        process.terminal_status_since = datetime.now()
...
def _trigger_sync_async() -> None:
    """
    Trigger async sync of all processes with pending documents.
    Uses threading to avoid blocking the request.
    Called when ANY process status changes.
    """
    import threading
    ...
    def _sync_in_thread():
        ...
        db = SessionLocal()
        try:
            SyncService.sync_all_processes_with_pending(db)
        except Exception:  # nosec: B110 - intentional silent fail for logging
            pass
        finally:
            db.close()

    thread = threading.Thread(target=_sync_in_thread, daemon=True)
    thread.start()
