...
def create_document(
    ...
    sync_state = DocumentSyncState(
        document_id=document.id, sync_status=SyncStatus.PENDING
    )
    db.add(sync_state)
    db.flush()
...
def update_document_validation(
    ...
    if old_status != status_str:
        logger.info(
            f"Validation changed for document {document_id}: {old_status} -> {status_str}"
        )
        document.process.pdf_needs_regeneration = True

    if notes is not None:
        document.validation_notes = notes or None
