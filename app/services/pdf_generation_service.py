...
def ensure_combined_pdf(db: Session, process_id: UUID) -> PDFEnsureResult:
    ...
    - Checking pdf_needs_regeneration flag to avoid unnecessary regeneration
    - Generating PDF if missing or validation changed
    ...
    if not process:
        ...
        return PDFEnsureResult(
            pdf=None, generated=False, skipped=True, skip_reason="process_not_found"
        )
    ...
    if combined_pdf:
        if not file_exists(combined_pdf.file_path):
            ...
            combined_pdf = None
        elif not process.pdf_needs_regeneration:
            logger.info(
                f"PDF exists and no validation changes for process {process_id}, skipping regeneration"
            )
            return PDFEnsureResult(pdf=combined_pdf, generated=False, skipped=False)
        else:
            logger.info(
                f"PDF exists but validation changed for process {process_id}, regenerating"
            )
    ...
    if pdf_path:
        combined_pdf = get_combined_pdf_for_process(db, process_id)
        if combined_pdf:
            process.pdf_needs_regeneration = False
            logger.info(
                f"Successfully generated PDF for process {process_id}, cleared regeneration flag"
            )
...
def batch_generate_pdfs(db: Session, force_regenerate: bool = False) -> List[Dict]:
    ...
    Args:
        ...
        force_regenerate: Se True, regera todos os PDFs mesmo que já existam.
                          Se False (padrão), apenas gera PDFs para processos
                          com pdf_needs_regeneration=True ou que têm arquivos
                          corrompidos (stale data).
    ...
    if not force_regenerate:
        processes = [p for p in processes if p.pdf_needs_regeneration]
        logger.info(
            f"Filtered processes needing PDF regeneration: {len(processes)}/{total_count}"
        )
