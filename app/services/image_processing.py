...
_conversion_semaphore = asyncio.Semaphore(
    getattr(settings, "MAX_CONCURRENT_IMAGE_CONVERSIONS", 3)
)


async def convert_image_to_pdf_async(image_bytes: bytes) -> bytes:
    """
    Convert image to PDF with concurrency protection.

    Uses semaphore to prevent OOM by limiting concurrent conversions.
    All processing happens in memory before writing to disk.
    ...
    async with _conversion_semaphore:
        return await asyncio.to_thread(convert_image_to_pdf, image_bytes)
