...
def send_status_notification(
    ...
    try:
        success, error_type = email_service.send_email(
            patient_email, subject, "status_update.html", context, db=db
        )
...
def _get_drs_email_config(db: Session, email_type: str) -> Tuple[str, str]:
    ...
    email_title = (
        f"REMESSA DE {today_str} - RENOVAÇÃO"
        if email_type == "renovacao"
        else f"REMESSA DE {today_str} - SOLICITAÇÃO"
    )
...
def send_drs_notification(
    ...
    reply_to = SettingsService.get_reply_to_email(db)
