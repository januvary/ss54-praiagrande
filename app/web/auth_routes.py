...
from app.utils.email_utils import get_email_link_url
...
async def login_submit(
    ...
    email_link_url = get_email_link_url(email, request)

    return render_template(
        request,
        "pages/check_email.html",
        {"email": email, "is_new_user": is_new_user, "email_link_url": email_link_url},
    )
        ...
        if user_id:
            with suppress(ValueError):
                ...
                if user:
                    ...
                    redirect_url = "/" if patients else "/select-patient"
                    return RedirectResponse(url=redirect_url, status_code=302)
