...
def _determine_desired_state(
    ...
    if enabled and existing:
        # Find the hour field from trigger.fields (it's a tuple, not a dict)
        hour_field = next(
            (f for f in existing.trigger.fields if f.name == "hour"), None
        )
        current_hour = hour_field.expressions[0].step if hour_field else None
        new_hour = new_config[job.hour_key]
        if current_hour is not None and current_hour != new_hour:
            return "reschedule"
    return "none"
...
def reload_scheduler_settings():
    ...
    try:
        ...
        if str(scheduler.timezone) != new_config["scheduler_timezone"]:
            logger.info(
                f"Timezone changed from {scheduler.timezone} to {new_config['scheduler_timezone']} - restarting scheduler"
            )
            shutdown_scheduler()
            init_scheduler(new_config)
